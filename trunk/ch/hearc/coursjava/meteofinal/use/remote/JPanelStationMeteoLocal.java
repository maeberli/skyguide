...
public class JPanelStationMeteoLocal extends JPanel {
	...
	private void startStopMeteoServive() throws Exception {
		...
		if (startStop.getText() == "Start") {
			if (meteoService == null) {
				...
				meteoService = MeteoServiceFactory.create(portsCom
						.getItemAt(portsCom.getSelectedIndex()));
				...
				MeteoServiceWrapper_I meteoServiceWrapper = new MeteoServiceWrapper_I() {
					...
					public void setMeteoServiceOptions(
						...
						sliderTemperatureDT.setValue((int) meteoServiceOptions
								.getTemperatureDT());
						sliderPressionDT.setValue((int) meteoServiceOptions
								.getPressionDT());
						sliderAltitudeDT.setValue((int) meteoServiceOptions
								.getAltitudeDT());
				...
				RmiURL rmiUrlMeteoService = new RmiURL(name, inetAddressLocal,
						RmiTools.PORT_RMI_DEFAUT);
				RmiTools.shareObject(meteoServiceWrapper, rmiUrlMeteoService);

				RmiURL rmiUrlAfficheurManager = new RmiURL(
						RMI_ID_AFFICHEUR_MANAGER, inetAddressServer,
						RmiTools.PORT_RMI_DEFAUT);

				AfficheurManager_I afficheurManager = (AfficheurManager_I) RmiTools
						.connectionRemoteObjectBloquant(rmiUrlAfficheurManager);

				affichageOptions = new AffichageOptions(
						(Integer) points.getValue(),
						inetAddressLocal.getHostAddress() + " - "
								+ title.getText());
				points.setEnabled(false);
				title.setEnabled(false);
				frameLocal.majTitle(this, affichageOptions.getTitre());

				RmiURL rmiUrlAfficheurService = afficheurManager
						.createRemoteAfficheurService(affichageOptions,
								new RmiURL(name, inetAddressLocal,
										RmiTools.PORT_RMI_DEFAUT));

				afficheurService = (AfficheurServiceWrapper_I) RmiTools
						.connectionRemoteObjectBloquant(new RmiURL(
								rmiUrlAfficheurService.getIdObjectRMI(),
								inetAddressServer, RmiTools.PORT_RMI_DEFAUT));
				afficheurService = (AfficheurServiceWrapper_I) RmiTools
						.connectionRemoteObjectBloquant(new RmiURL(
								rmiUrlAfficheurService.getIdObjectRMI(),
								inetAddressServer, RmiTools.PORT_RMI_DEFAUT));
				...
				meteoService.addMeteoListener(new MeteoListener_I() {
					...
					public void temperaturePerformed(MeteoEvent event) {
						panelGrapheTemperature.getGraphe().addToDataset(
								event.getTime(), event.getValue());
					...
					public void pressionPerformed(MeteoEvent event) {
						panelGraphePression.getGraphe().addToDataset(
								event.getTime(), event.getValue());
					...
					public void altitudePerformed(MeteoEvent event) {
						panelGrapheAltitude.getGraphe().addToDataset(
								event.getTime(), event.getValue());
				...
				frameLocal.getPortInUseMap().put(
						portsCom.getItemAt(portsCom.getSelectedIndex()), true);
				frameLocal.deleteInUsePortsInOtherPanels(this,
						portsCom.getItemAt(portsCom.getSelectedIndex()));
				meteoService.start(meteoServiceOptions);
				startStop.setText("Stop");
				isConnecting.setForeground(Color.GREEN);
				isRunning.setForeground(Color.GREEN);
		...
		{
			frameLocal.getPortInUseMap().put(
					portsCom.getItemAt(portsCom.getSelectedIndex()), false);
			meteoService.stop();
	...
	private void geometrie() {
		...
		panelGrapheTemperature = new PanelGraphe("Temperature", "Temps",
				"Temperature");
		...
		meteoServicePanel.add(new JPanelTitle("Meteo service remote"),
				BorderLayout.NORTH);
		...
		meteoServiceOptionsPanel.add(new JPanelTitle("Meteo service options"),
				BorderLayout.NORTH);
		...
		meteoAffichageOptionsPanel.add(new JPanelTitle(
				"Meteo affichage options"), BorderLayout.NORTH);
	...
	private void controle() {
		...
		sliderTemperatureDT.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent event) {
				meteoServiceOptions.setTemperatureDT(sliderTemperatureDT
						.getValue());
				try {
					afficheurService
							.setMeteoServiceOptions(meteoServiceOptions);
